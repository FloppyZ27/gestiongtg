...
const PriseDeMandat = React.forwardRef(({ filterPlaceAffaire = "tous", filterEquipeExternal = "Toutes", onActiveTabChange }, ref) => {
  ...
  const buildClientInitialData=(type)=>{const sp=(s)=>{const p=(s||"").trim().split(" ");return{prenom:p.length>1?p.slice(0,-1).join(" "):p[0]||"",nom:p.length>1?p[p.length-1]:""};};if(type==="Client"){let prenom=clientInfo.prenom||"",nom=clientInfo.nom||"",telephone=clientInfo.telephone||"",type_telephone=clientInfo.type_telephone||"Cellulaire",courriel=clientInfo.courriel||"";if(!prenom&&!nom&&!telephone&&!courriel&&formData.clients_ids?.length>0){const fc=clients?.find(c=>c.id===formData.clients_ids[0]);if(fc){prenom=fc.prenom||"";nom=fc.nom||"";const t=fc.telephones?.find(x=>x.actuel||x.actuelle)||fc.telephones?.[0];if(t){telephone=t.telephone||"";type_telephone=t.type||"Cellulaire";}const e=fc.courriels?.find(x=>x.actuel||x.actuelle)||fc.courriels?.[0];if(e)courriel=e.courriel||"";}}return{prenom,nom,telephone,type_telephone,courriel,adresse_travaux:(workAddress.rue||workAddress.ville)?workAddress:null};}if(type==="Notaire")return{...sp(professionnelInfo.notaire),telephone:professionnelInfo.notaire_telephone||"",courriel:professionnelInfo.notaire_courriel||""};if(type==="Courtier immobilier")return{...sp(professionnelInfo.courtier),telephone:professionnelInfo.courtier_telephone||"",courriel:professionnelInfo.courtier_courriel||""};if(type==="Compagnie")return{prenom:professionnelInfo.compagnie||"",nom:"",telephone:"",courriel:""};return null;};
