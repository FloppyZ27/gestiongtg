...
const PriseDeMandat = React.forwardRef(({ filterPlaceAffaire = "tous", filterEquipeExternal = "Toutes", onActiveTabChange }, ref) => {
  ...
  const handleAutoSave = async (overrideCommentaires, overrideAddress) => {
    ...
    if (!overrideCommentaires && !overrideAddress && !hasFormChanges) return; if(updatePriseMandatMutation.isPending) return;
  ...
  const buildClientInitialData=(type)=>{const sp=(s)=>{const p=(s||"").trim().split(" ");return{prenom:p.length>1?p.slice(0,-1).join(" "):p[0]||"",nom:p.length>1?p[p.length-1]:""};};if(type==="Client"){const ci=editingPriseMandat?.client_info||{};let prenom=ci.prenom||clientInfo.prenom||"",nom=ci.nom||clientInfo.nom||"",telephone=ci.telephone||clientInfo.telephone||"",type_telephone=ci.type_telephone||clientInfo.type_telephone||"Cellulaire",courriel=ci.courriel||clientInfo.courriel||"";if(!prenom&&!nom&&!telephone&&!courriel){const ids=(editingPriseMandat?.clients_ids||formData.clients_ids||[]);if(ids.length>0){const fc=clients?.find(c=>c.id===ids[0]);if(fc){prenom=fc.prenom||"";nom=fc.nom||"";const t=fc.telephones?.find(x=>x.actuel||x.actuelle)||fc.telephones?.[0];if(t){telephone=t.telephone||"";type_telephone=t.type||"Cellulaire";}const e=fc.courriels?.find(x=>x.actuel||x.actuelle)||fc.courriels?.[0];if(e)courriel=e.courriel||"";}}}const wa=editingPriseMandat?.adresse_travaux||workAddress;const adresse_travaux=(wa?.rue||wa?.ville)?wa:null;console.log("PREFILL",{prenom,nom,telephone,courriel});return{prenom,nom,telephone,type_telephone,courriel,adresse_travaux};}if(type==="Notaire"){const pi=editingPriseMandat?.professionnel_info||professionnelInfo;const name=pi.notaire||"";const ids=editingPriseMandat?.notaires_ids||formData.notaires_ids||[];const fc=ids.length>0?clients?.find(c=>c.id===ids[0]):null;const tel=fc?.telephones?.find(x=>x.actuel||x.actuelle)||fc?.telephones?.[0];const em=fc?.courriels?.find(x=>x.actuel||x.actuelle)||fc?.courriels?.[0];return{...sp(fc?`${fc.prenom||''} ${fc.nom||''}`.trim():name),telephone:tel?.telephone||"",type_telephone:tel?.type||"Cellulaire",courriel:em?.courriel||""};}if(type==="Courtier immobilier"){const pi=editingPriseMandat?.professionnel_info||professionnelInfo;const name=pi.courtier||"";const ids=editingPriseMandat?.courtiers_ids||formData.courtiers_ids||[];const fc=ids.length>0?clients?.find(c=>c.id===ids[0]):null;const tel=fc?.telephones?.find(x=>x.actuel||x.actuelle)||fc?.telephones?.[0];const em=fc?.courriels?.find(x=>x.actuel||x.actuelle)||fc?.courriels?.[0];return{...sp(fc?`${fc.prenom||''} ${fc.nom||''}`.trim():name),telephone:tel?.telephone||"",type_telephone:tel?.type||"Cellulaire",courriel:em?.courriel||""};}if(type==="Compagnie"){const ids=editingPriseMandat?.compagnies_ids||formData.compagnies_ids||[];const fc=ids.length>0?clients?.find(c=>c.id===ids[0]):null;const pi=editingPriseMandat?.professionnel_info||professionnelInfo;const name=fc?fc.prenom||fc.nom||"":pi.compagnie||"";const tel=fc?.telephones?.find(x=>x.actuel||x.actuelle)||fc?.telephones?.[0];const em=fc?.courriels?.find(x=>x.actuel||x.actuelle)||fc?.courriels?.[0];return{prenom:name,nom:"",telephone:tel?.telephone||"",courriel:em?.courriel||""};}return null;};
          ...
          <Dialog open={isDialogOpen} onOpenChange={async (open) => {
            if (!open) {
              ...
              const hasChanges = 
                ...
                (formData.clients_ids?.length ?? 0) > 0 ||
                (formData.notaires_ids?.length ?? 0) > 0 ||
                (formData.courtiers_ids?.length ?? 0) > 0 ||
                (formData.compagnies_ids?.length ?? 0) > 0 ||
                clientInfo.prenom ||
                clientInfo.nom ||
            ...
            <DialogContent className={`border-0 text-white max-w-[75vw] w-[75vw] p-0 gap-0 overflow-hidden shadow-2xl shadow-black/50`} style={{ marginTop: '19px', maxHeight: 'calc(85vh - 5px)' }}>
