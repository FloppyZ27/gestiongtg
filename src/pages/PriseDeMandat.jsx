...
const PriseDeMandat = React.forwardRef((props, ref) => {
  ...
  const handleAutoSave = async (overrideCommentaires, overrideAddress) => {
    ...
    if(_ncc>_occ){const strip=(h)=>h?.replace(/<[^>]*>/g,'').replace(/\[IMAGE:[^\]]*\]/g,'Image').replace(/\[AUDIO:[^\]]*\]/g,'Audio').trim()||'';const isMedia=(c)=>c?.contenu?.includes('<img ')||c?.contenu?.includes('<audio ')||c?.contenu?.startsWith('data:image')||c?.contenu?.startsWith('data:audio')||c?.contenu?.startsWith('[IMAGE:')||c?.contenu?.startsWith('[AUDIO:');const newComments=currentCommentaires.slice(0,_ncc-_occ);newComments.forEach(c=>{if(isMedia(c)){_h('Commentaire ajouté',(c?.contenu?.includes('<img ')||c?.contenu?.startsWith('data:image')||c?.contenu?.startsWith('[IMAGE:'))?'Image':'Audio');}else{const txt=strip(c.contenu);const preview=txt.length>120?txt.substring(0,120)+'...':txt;_h('Commentaire ajouté',preview||'(commentaire vide)');}}); }
    else if(_ncc<_occ){const strip2=(h)=>h?.replace(/<[^>]*>/g,'').replace(/\[IMAGE:[^\]]*\]/g,'Image').replace(/\[AUDIO:[^\]]*\]/g,'Audio').trim()||'';const isMedia2=(c)=>c?.contenu?.includes('<img ')||c?.contenu?.includes('<audio ')||c?.contenu?.startsWith('data:image')||c?.contenu?.startsWith('data:audio')||c?.contenu?.startsWith('[IMAGE:')||c?.contenu?.startsWith('[AUDIO:');const oldComms=initialPriseMandatData.commentaires||[];const removed=oldComms.filter(oc=>!currentCommentaires.some(nc=>nc.contenu===oc.contenu));if(removed.length>0){removed.forEach(c=>{if(isMedia2(c)){_h('Commentaire supprimé',(c?.contenu?.includes('<img ')||c?.contenu?.startsWith('data:image')||c?.contenu?.startsWith('[IMAGE:'))?'Image':'Audio');}else{const txt=strip2(c.contenu);const preview=txt.length>120?txt.substring(0,120)+'...':txt;_h('Commentaire supprimé',preview||'(commentaire vide)');}});}else{_h('Commentaire supprimé',`${_occ-_ncc} commentaire(s) supprimé(s)`);}}
    else{const oldC=(initialPriseMandatData.commentaires||[]).map(c=>c.contenu);const newC=currentCommentaires.map(c=>c.contenu);const strip=(h)=>h?.replace(/<[^>]*>/g,'').replace(/\[IMAGE:[^\]]*\]/g,'Image').replace(/\[AUDIO:[^\]]*\]/g,'Audio').trim()||'';for(let i=0;i<Math.min(oldC.length,newC.length);i++){if(oldC[i]!==newC[i]){const ot=strip(oldC[i]).substring(0,80)+(strip(oldC[i]).length>80?'...':'');const nt=strip(newC[i]).substring(0,80)+(strip(newC[i]).length>80?'...':'');_h('Commentaire modifié',`"${ot}" → "${nt}"`);}}}
