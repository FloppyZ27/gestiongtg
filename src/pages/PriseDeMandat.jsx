...
const PriseDeMandat = React.forwardRef((props, ref) => {
  ...
  const handleAutoSave = async (overrideCommentaires, overrideAddress) => {
    ...
    if(_ncc>_occ){const strip=(h)=>h?.replace(/<[^>]*>/g,'').trim()||'';const newComments=currentCommentaires.slice(_occ);newComments.forEach(c=>{const txt=strip(c.contenu);const preview=txt.length>120?txt.substring(0,120)+'...':txt;_h("Commentaire ajouté",preview||'(commentaire vide)');});}
