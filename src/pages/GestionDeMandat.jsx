               ...
               <div className="overflow-x-auto w-full -mx-4 md:-mx-8">
                 <div className="flex gap-4 p-4" style={{ width: 'fit-content', minWidth: '100%' }}>
                    ...
                    return (
                      ...
                      >
                        <Card 
                           className={`${getTacheColor(tache)} bg-slate-900/50 backdrop-blur-xl shadow-xl flex flex-col`}
                        >
                          <CardHeader className={`pb-3 pt-3 border-b-2 border-slate-800`} style={{ background: `linear-gradient(to right, ${getTacheHeaderColor(tache).includes('blue') ? 'rgba(59, 130, 246, 0.2)' : getTacheHeaderColor(tache).includes('cyan') ? 'rgba(6, 182, 212, 0.2)' : getTacheHeaderColor(tache).includes('purple') ? 'rgba(168, 85, 247, 0.2)' : getTacheHeaderColor(tache).includes('green') ? 'rgba(34, 197, 94, 0.2)' : getTacheHeaderColor(tache).includes('yellow') ? 'rgba(234, 179, 8, 0.2)' : getTacheHeaderColor(tache).includes('orange') ? 'rgba(249, 115, 22, 0.2)' : getTacheHeaderColor(tache).includes('pink') ? 'rgba(236, 72, 153, 0.2)' : getTacheHeaderColor(tache).includes('indigo') ? 'rgba(99, 102, 241, 0.2)' : getTacheHeaderColor(tache).includes('teal') ? 'rgba(20, 184, 166, 0.2)' : getTacheHeaderColor(tache).includes('red') ? 'rgba(239, 68, 68, 0.2)' : getTacheHeaderColor(tache).includes('amber') ? 'rgba(245, 158, 11, 0.2)' : 'rgba(16, 185, 129, 0.2)'}, ${getTacheHeaderColor(tache).includes('blue') ? 'rgba(37, 99, 235, 0.2)' : getTacheHeaderColor(tache).includes('cyan') ? 'rgba(8, 145, 178, 0.2)' : getTacheHeaderColor(tache).includes('purple') ? 'rgba(147, 51, 234, 0.2)' : getTacheHeaderColor(tache).includes('green') ? 'rgba(22, 163, 74, 0.2)' : getTacheHeaderColor(tache).includes('yellow') ? 'rgba(202, 138, 4, 0.2)' : getTacheHeaderColor(tache).includes('orange') ? 'rgba(234, 88, 12, 0.2)' : getTacheHeaderColor(tache).includes('pink') ? 'rgba(219, 39, 119, 0.2)' : getTacheHeaderColor(tache).includes('indigo') ? 'rgba(79, 70, 229, 0.2)' : getTacheHeaderColor(tache).includes('teal') ? 'rgba(13, 148, 136, 0.2)' : getTacheHeaderColor(tache).includes('red') ? 'rgba(220, 38, 38, 0.2)' : getTacheHeaderColor(tache).includes('amber') ? 'rgba(217, 119, 6, 0.2)' : 'rgba(5, 150, 105, 0.2)'})` }}>
                ...
                <div className="flex gap-4 p-4 min-w-max">
                  ...
                  {usersList.map((user, userIndex) => {
                    ...
                    return (
                      ...
                      >
                        <Card 
                           className={`${bgColor} ${borderColor} bg-slate-900/50 backdrop-blur-xl shadow-xl flex flex-col`}
                        >
