...
export default function GestionDeMandat() {
  ...
  const renderColumn = (columnId, title, cards, headerContent) => {
    ...
    return (
      ...
      >
        <div className={`rounded-xl border transition-all duration-150 flex flex-col ${isOver ? 'border-emerald-400/80 bg-emerald-500/10 shadow-lg shadow-emerald-500/20' : 'border-transparent bg-transparent'}`}>
      ...
      <div className="w-full">
        <div className="pb-4 pt-4">
          <div className="flex items-center gap-3 mb-2">
            ...
            <Kanban className="w-6 h-6" style={{ color: '#ff6b35' }} />
          </div>
                  ...
                  {[{ val: "Toutes", label: "Toutes" }, { val: "Alma", label: "Alma" }, { val: "Saguenay", label: "Saguenay" }].map(({ val, label }) => {
                    ...
                    return (
                      <button key={val} onClick={() => setFilterPlaceAffaire(val)}
                        className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all border-0 ${isActive ? "bg-orange-500/20 text-orange-400 border-b-2 border-orange-400" : "text-slate-400 hover:text-slate-200 hover:bg-slate-800/50"}`}>
                        {label}
                        <span className={`ml-1.5 text-xs px-1.5 py-0.5 rounded-full ${isActive ? "bg-red-500/30 text-orange-300" : "bg-slate-700 text-slate-400"}`}>{count}</span>
                      </button>
                    );
                  })}
                </div>
                  ...
                  <div className="flex justify-between items-center gap-2">
                    ...
                    <div className="flex items-center gap-3">
                         <Button
                           ...
                           className="bg-slate-800 border-slate-700 text-white hover:bg-slate-700 hover:scale-105 transition-all duration-200 h-8 w-8 p-0"
                         >
                         ...
                         <Button size="sm" variant="outline" onClick={() => setCurrentMonthStart(calendarMode === "week" ? startOfWeek(new Date(), { weekStartsOn: 1 }) : startOfMonth(new Date()))} className="bg-primary/20 text-primary border-primary/30 transition-all duration-200 hover:bg-primary/40 hover:scale-105">Aujourd'hui</Button>
                         </div>

                         {/* Droite : toggle Semaine/Mois */}
                         <div className="flex gap-1">
                         <Button size="sm" variant="outline" onClick={() => { setCalendarMode("week"); setCurrentMonthStart(startOfWeek(new Date(), { weekStartsOn: 1 })); }} className={`transition-all duration-200 hover:scale-105 ${calendarMode === "week" ? "border-primary text-white hover:bg-primary/10" : "border-slate-700 text-white hover:bg-slate-800"}`}>Semaine</Button>
                         <Button size="sm" variant="outline" onClick={() => { setCalendarMode("month"); setCurrentMonthStart(startOfMonth(new Date())); }} className={`transition-all duration-200 hover:scale-105 ${calendarMode === "month" ? "border-primary text-white hover:bg-primary/10" : "border-slate-700 text-white hover:bg-slate-800"}`}>Mois</Button>
                         </div>
                  </div>
                </CardHeader>
