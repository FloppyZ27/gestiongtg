          ...
          <div className="flex-1 overflow-y-auto p-6">
            ...
            {editingDossier && (
              <Card className="border-slate-700 bg-slate-800/30 mt-3" data-section="entree-temps">
                <CardHeader className="cursor-pointer hover:bg-lime-900/40 transition-colors rounded-t-lg py-1.5 bg-lime-900/20" onClick={() => setEntreeTempsCollapsed(!entreeTempsCollapsed)}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      {entreesTemps.length > 0 && <Badge className="bg-lime-500/20 text-lime-400 border-lime-500/30 text-xs font-semibold">Total: {entreesTemps.reduce((sum, e) => sum + (e.heures || 0), 0)}h</Badge>}
                    </div>
                ...
                {!entreeTempsCollapsed && (
                  <CardContent className="pt-2 pb-3 space-y-4">
                    {entreesTemps.length > 0 ? (
                          ...
                          <TableBody>
                            {entreesTemps.map((entree) => (
                              <TableRow key={entree.id} className="hover:bg-slate-800/30 border-slate-800">
                                ...
                                <TableCell><Badge className={`${getMandatColor(entree.mandat)} border text-xs`}>{getAbbreviatedMandatType(entree.mandat) || "-"}</Badge></TableCell>
                                <TableCell><Badge className="bg-yellow-500/20 text-yellow-400 border-yellow-500/30 text-xs">{entree.tache || "-"}</Badge></TableCell>
                                <TableCell className="text-slate-300 text-xs">
                                ...
                                <TableCell className="text-right">
                                  <button type="button" onClick={() => { setMinuteToDeleteInfo({entreeTempsId: entree.id}); setShowDeleteMinuteConfirm(true); }} className="text-slate-400 hover:text-red-400 transition-colors"><Trash className="w-4 h-4" /></button>
                                </TableCell>
                              </TableRow>
                            ))}
                          </TableBody>
                      ...
                      <div className="text-center py-6 text-slate-500"><Clock className="w-8 h-8 mx-auto mb-2 text-slate-600" /><p className="text-sm">Aucune entrée de temps</p></div>
                    )}
                  </CardContent>
                )}
              </Card>
