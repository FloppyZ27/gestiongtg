      ...
      >
        <div className="flex items-start justify-between gap-2 mb-2">
          ...
          <div className="grid grid-cols-2 gap-0.5 flex-shrink-0">
            ...
            {showLock && <Button size="sm" onClick={(e) => { e.stopPropagation(); toggleLockCard(card.id); }} style={{ background: isLocked ? '#eab308' : 'rgba(59, 130, 246, 0.3)', color: isLocked ? '#ffffff' : '#3b82f6', transition: 'all 150ms' }} onMouseEnter={(e) => { e.currentTarget.style.background = '#eab308'; e.currentTarget.style.color = '#ffffff'; }} onMouseLeave={(e) => { e.currentTarget.style.background = isLocked ? '#eab308' : 'rgba(59, 130, 246, 0.3)'; e.currentTarget.style.color = isLocked ? '#ffffff' : '#3b82f6'; }} className="h-5 w-5 p-0" title={isLocked ? 'Déverrouiller' : 'Verrouiller'}>{isLocked ? <Lock className="w-2.5 h-2.5" /> : <Unlock className="w-2.5 h-2.5" />}</Button>}
          </div>
        </div>
