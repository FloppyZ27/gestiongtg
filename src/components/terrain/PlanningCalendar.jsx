      ...
      >
        <div className="flex items-start justify-between gap-2 mb-2">
          ...
          <div className="grid grid-cols-2 gap-0.5 flex-shrink-0">
            ...
            {showLock && <Button size="sm" onClick={(e) => { e.stopPropagation(); toggleLockCard(card.id); }} style={{ background: isLocked ? '#fbbf24' : 'rgba(59, 130, 246, 0.4)', color: isLocked ? '#1a1a1a' : '#60a5fa', transition: 'all 150ms' }} onMouseEnter={(e) => { e.currentTarget.style.background = '#fbbf24'; e.currentTarget.style.color = '#1a1a1a'; }} onMouseLeave={(e) => { e.currentTarget.style.background = isLocked ? '#fbbf24' : 'rgba(59, 130, 246, 0.4)'; e.currentTarget.style.color = isLocked ? '#1a1a1a' : '#60a5fa'; }} className="h-5 w-5 p-0" title={isLocked ? 'Déverrouiller' : 'Verrouiller'}>{isLocked ? <Lock className="w-2.5 h-2.5" /> : <Unlock className="w-2.5 h-2.5" />}</Button>}
          </div>
        </div>
