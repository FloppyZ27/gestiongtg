...
export default function ClientFormDialog({ 
  ...
  const { data: commentairesCount = [] } = useQuery({
    queryKey: ['commentairesClient', editingClient?.id],
  ...
  React.useEffect(() => {
    ...
    if (editingClient) {
      ...
      // Mode création — lire initialData directement (le remount par key garantit qu'il est frais)
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                          ...
                          <div className={`border-none rounded-lg overflow-hidden ${courrielDisabled ? 'opacity-50' : ''}`}>
                            <Table>
                          <TableHeader>
                          <TableRow className="bg-slate-800/50 hover:bg-slate-800/50 border-slate-700">
              ...
              <div className="p-4 border-b border-slate-800 flex-shrink-0">
                <TabsList className="grid grid-cols-2 h-9 w-full bg-transparent gap-2">
                  <TabsTrigger value="commentaires" className="text-xs bg-transparent border-none data-[state=active]:text-emerald-400 data-[state=inactive]:text-slate-400 hover:text-emerald-300 flex items-center gap-1">
                    ...
                    Commentaires
                    {nbCommentaires > 0 && <span className="ml-1 px-1.5 py-0.5 rounded-full text-xs bg-orange-500/20 text-orange-400 border border-orange-500/30 font-semibold pointer-events-none">{nbCommentaires}</span>}
                  </TabsTrigger>
